...
class ProductionExecutionController extends Controller
{
    ...
    public function start(Request $request, $productionOrderId)
    {
        ...
        if ($po->status !== 'released') {
            return response()->json(['message' => 'Production order harus released terlebih dahulu'], 422);
        }
        ...
        try {
            ...
            return response()->json(['message' => 'Produksi dimulai, material berhasil dikonsumsi']);

        } catch (\Exception $e) {
            return response()->json(['message' => 'Gagal memulai produksi', 'error' => $e->getMessage()], 500);
        }
    }

    /**
     * POST - Complete Production
     * ↳ Otomatis update product.harga = hpp_per_unit setelah selesai
     */
    public function complete(Request $request, $productionOrderId)
    {
        ...
        if ($po->status !== 'in_progress') {
            return response()->json(['message' => 'Production order harus in_progress'], 422);
        }
        ...
        try {
            $hppPerUnit = DB::transaction(function () use ($po, $validated) {
                $totalProductionCost = $po->total_material_cost
                ...
                // ── Otomatis update harga produk dari HPP ──────────────────
                $po->product->update([
                    'harga'          => $hppPerUnit,
                    'hpp_terakhir'   => $hppPerUnit,
                    'hpp_updated_at' => now(),
                ]);
                ...
                Stock::firstOrCreate(
                    ...
                    ['quantity'   => 0]
                )->increment('quantity', $validated['quantity_actual']);
            ...
            return response()->json([
                ...
                'hpp_per_unit' => $hppPerUnit,
                'harga_produk_diperbarui' => number_format($hppPerUnit, 2),
            ]);

        } catch (\Exception $e) {
            return response()->json(['message' => 'Gagal menyelesaikan produksi', 'error' => $e->getMessage()], 500);
        }
    }

    /**
     * GET - Production Report (tidak ada perubahan dari sebelumnya)
     */
    public function getReport($productionOrderId)
    {
        $po = ProductionOrder::with(['product', 'bom', 'materialUsages.rawMaterial'])
            ->findOrFail($productionOrderId);

        if ($po->status !== 'completed') {
            return response()->json(['message' => 'Production order belum completed'], 422);
        }
        ...
        return response()->json([
            ...
            'harga_produk_saat_ini' => $po->product->harga,
            'material_usage'        => $po->materialUsages->map(fn($u) => [
                'material'      => $u->rawMaterial->name,
    ...
    /**
     * GET - List all production executions (tidak ada perubahan)
     */
